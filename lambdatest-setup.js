...
const modifyCapabilities = (configName, testName) => {
  ...
  if (configName.match(/android/)) {
    ...
    capabilities['LT:Options']['build'] = 'Playwright JS Build 01'
  ...
  page: async ({ page, playwright }, use, testInfo) => {
    ...
    if (testInfo.project.name.match(/lambdatest/)) {
      ...
      if (testInfo.project.name.match(/android/)) {
        // Android test
        device = await _android.connect(`ws://cdp.lambdatest.com/playwright?capabilities=${encodeURIComponent(JSON.stringify(capabilities))}`);
        ...
        // Desktop test
        browser = await chromium.connect(`ws://cdp.lambdatest.com/playwright?capabilities=${encodeURIComponent(JSON.stringify(capabilities))}`)
